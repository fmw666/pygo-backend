    ...
    with settings.DB.atomic() as txn:
        try:
            inv_history = InventoryHistory.get(
                InventoryHistory.order_sn == order_sn,
            ...
            for item in inv_details:
                ...
                Inventory.update(stocks=Inventory.stocks+num).where(
                    Inventory.goods == goods_id).execute()
    ...
    def Sell(
        ...
        with settings.DB.atomic() as txn:
            for item in request.goodsInfo:
                lock = Lock(settings.REDIS_CLIENT,
                            f"lock:goods:{item.goodsId}",
                            auto_renewal=True, expire=10)
                lock.acquire()
                goods_inv = Inventory.select().where(
                    Inventory.goods == item.goodsId).first()
                # 库存不存在或者库存不足
                if not goods_inv:
                    context.set_code(grpc.StatusCode.NOT_FOUND)
                    context.set_details("inventory not found")
