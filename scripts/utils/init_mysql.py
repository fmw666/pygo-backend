...
from scripts.utils.common import get_mysql_cursor, config
...
def create_db() -> None:
    ...
    try:
        dbs_name = [config.Mysql.goods_srv_db, config.Mysql.order_srv_db, config.Mysql.inventory_srv_db, \
                    config.Mysql.user_srv_db, config.Mysql.userop_srv_db]
...
def create_table_and_exec_sql() -> None:
    ...
    for srv in srvs_list:
        # find pkg
        py_file_path = os.path.join(root_dir, "backend", "services", srv, "model", "models.py")
        if not os.path.exists(py_file_path):
            raise Exception(f"not found {py_file_path}")
