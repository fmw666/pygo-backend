...
def serve() -> None:
    ...
    logger.add("logs/inventory_srv_{time}.log", rotation="500 MB",
               encoding="utf-8")

    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    # register inventory service
    inventory_pb2_grpc.add_InventoryServicer_to_server(InventoryServicer(),
                                                       server)
    ...
    # register service
    logger.info("register inventory service to consul")
    register = consul.ConsulRegister(settings.CONSUL_HOST,
                                     settings.CONSUL_PORT)
    if not register.register(
        ...
        logger.error("register inventory service to consul failed")
        sys.exit(0)
