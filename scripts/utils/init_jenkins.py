...
from scripts.utils.common import get_jenkins_client, config
...
def get_admin_password() -> str:
    ...
    pswd_file_path = os.path.join(root_dir, "dockerfiles", "data", "jenkins", "secrets", "initialAdminPassword")
    password = ""
    with open(pswd_file_path, "r") as f:
        password = f.read().strip()
    
    if password == "":
        raise Exception("获取初始密码失败. 请检查文件路径是否正确. 正确路径：/var/jenkins_home/secrets/initialAdminPassword")

    return password


def get_crumb(session: requests.Session | None = None, auth: tuple | None = None) -> str:
...
def unlock_jenkins() -> None:
    ...
    unlock_url = f"http://{config.DockerServer.host}:{config.Jenkins.port}/j_spring_security_check"
    data = {
        "from": "",
        "j_username": "admin",
        "j_password": password,
...
def request_install_plugin_api(plugin_name: str, plugin_url: str, session: requests.Session) -> requests.Response:
    headers = {
...
def install_plugins() -> None:
    # Localization: Chinese (Simplified), SSH Credentials, Publish Over SSH, Pipeline, Git
    ...
    plugin_names = ["Localization: Chinese (Simplified)@1.0.24", \
                    "SSH Credentials Plugin@308.ve4497b_ccd8f4", "Publish Over SSH@1.25", \
                    "Pipeline@596.v8c21c963d92d", "git@5.2.0"]
...
def create_global_credential() -> None:
    ...
    response = requests.post(f"{jenkins_url}/credentials/store/system/domain/_/createCredentials",
...
def create_job(client) -> None:
    jobs_name_dict = {
        "srv": {
            ...
            "jobs_name": [
                "goods-srv-pipeline", "inventory-srv-pipeline", "order-srv-pipeline", \
                "user-srv-pipeline", "userop-srv-pipeline"
            ]
        },
        "api": {
    ...
    for job_type, job_info in jobs_name_dict.items():
        ...
        xml_path = os.path.join(parent_dir, "jenkinscfg", f"job_pipeline_{job_type}.xml")

        for job_name in jobs_name:
            # xxx-web-pipeline -> xxx_web,  xxx-srv-pipeline -> xxx_srv
            proj_folder_name = job_name.replace("-pipeline", "").replace("-", "_")
            modify_job_xml_scriptPath(xml_path, f"{proj_folder_name}/Jenkinsfile")
            xml = open(xml_path, "r", encoding="utf-8").read()
            ...
            if client.job_exists(name=job_name_with_folder):
                ...
                while client.job_exists(new_job_name):
                    ...
                    rename_count += 1
                client.rename_job(from_name=job_name_with_folder, to_name=new_job_name)
...
def execute():
    try:
        ...
        raise Exception("""连接 Jenkins 失败. 失败原因可能如下:
                        ...
                        2. Jenkins 用户身份认证失败，请确保 config.ini 中 Jenkins 用户已在管理台正确创建.""")
    else:
